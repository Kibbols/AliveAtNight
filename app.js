...
function showPollButton() {
  ...
  btn.onclick = async () => {
    ...
    try {
      const extracted = await callGemini(`You are a data extractor for a Twitch poll. Given the following Dead by Daylight build recommendations, extract the build name, perk names, and add-on names for each build. Then create a "poll_choice" for each build: a string of the 4 perk names abbreviated to be human-readable, separated by commas with NO spaces after commas. The string must be EXACTLY as close to 25 characters as possible without exceeding 25. Use the full budget — aim for 20-25 characters. Rules: replace "Hex:" with "H:", use the longest readable abbreviation that fits (e.g. prefer "Pop" over "P", "Corrupt" over "Crpt", "Deadlock" over "DL", "Nowhere to Hide" -> "NtHide", "Save the Best for Last" -> "StBfL"). If you have characters to spare, spell more of each name out. The entire poll_choice string including commas must be 25 characters or fewer. Return ONLY a JSON array with no markdown, no backticks, no explanation. Format exactly like this:
[
